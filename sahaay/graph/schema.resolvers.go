...
// 🕕 PURPOSE: Converts Go time.Time to GraphQL string format
...
// ⏰ PURPOSE: Converts optional expiration time to GraphQL string format
...
// 🚨 THIS IS THE CORE FUNCTIONALITY OF SAHAAY 🚨
...
func (r *mutationResolver) CreateDistressMessage(ctx context.Context, input model.CreateDistressMessageInput) (*model.DistressMessage, error) {
	...
	distressMessage := &model.DistressMessage{
		...
		Status:       model.MessageStatusPending, // Lifecycle: PENDING → DELIVERED → ACKNOWLEDGED
	...
	fmt.Printf("🚨 EMERGENCY CREATED: %s %s at GPS(%.4f, %.4f) - %s\n", 
...
// 🔍 PURPOSE: Shows ALL active emergency requests in the system
...
func (r *queryResolver) DistressMessages(ctx context.Context) ([]*model.DistressMessage, error) {
	...
	fmt.Printf("📊 Retrieved %d active distress messages from database\n", len(messages))
